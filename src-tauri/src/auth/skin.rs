...
pub async fn get_head_png(skin_url: &str) -> Result<Vec<u8>> {
    info!("[skin] Descargando skin desde: {}", skin_url);
    ...
    info!(
        "[skin] Skin descargada ({} bytes), extrayendo cabeza...",
        bytes.len()
    );
    ...
    let scaled = image::imageops::resize(&head, 64, 64, image::imageops::FilterType::Nearest);
    ...
    info!("[skin] ✓ Cabeza extraída ({} bytes PNG)", png_bytes.len());
    Ok(png_bytes)
}
...
pub async fn get_head_base64(skin_url: &str) -> Result<String> {
    ...
    info!("[skin] ✓ Cabeza convertida a base64 ({} bytes)", png.len());
    Ok(format!(
